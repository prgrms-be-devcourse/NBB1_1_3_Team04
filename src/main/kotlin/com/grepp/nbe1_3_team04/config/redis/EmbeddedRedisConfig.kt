...
class EmbeddedRedisConfig(
    ...
    fun redisServer() {
        val port: Int = if (isRedisRunning) findAvailablePort() else redisPort
        if (isArmMac) {
    ...
    private val isArmMac: Boolean
        get() = System.getProperty("os.arch") == "aarch64"
    ...
    private fun executeGrepProcessCommand(port: Int): Process {
        ...
        if (OS.contains("win")) {
            val command: String = String.format("netstat -nao | find \"LISTEN\" | find \"%d\"", port)
            val shell: Array<String> = arrayOf("cmd.exe", "/y", "/c", command)
            return Runtime.getRuntime().exec(shell)
        }
        val command: String = String.format("netstat -nat | grep LISTEN|grep %d", port)
    ...
    private fun isRunning(process: Process): Boolean {
        var line: String?
        val pidInfo: StringBuilder = StringBuilder()
